...
Legacy GitHub Pages serves the repo root directly; there is no build step and no CI. After ANY edit to `index.html`, `atlas.html`, or `dementia.html`, run `python gen_mirrors.py` and commit the regenerated mirrors (`index.md`, `atlas.md`, `dementia.md`, their `.txt` twins, `llms-full.txt`, `full_site.txt`). JavaScript is enhancement only in ONE sense: everything the page shows must also exist as text in the mirrors (text parity is the law). Beyond that, use JavaScript freely wherever it makes the better page — the publisher has said so explicitly; do not contort designs to avoid JS. The agent-facing laws are in `denson/stoagen-com` at `docs/building-agent-friendly-sites.md`.
